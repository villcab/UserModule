...
public class PerfilBean implements Serializable {
    ...
    public void editar() {
	Long id = Long.parseLong(FacesUtil.getParametro("perfilId").toString());
	perfil = selectFirst(listPerfiles, having(on(Perfil.class).getPerfilId(), equalTo(id)));
	...
	if (!edit) {	//NUEVO
		...
		FacesUtil.showMessage("Perfil guardado correctamente", FacesUtil.INFO);
		
	    } catch (Exception e) {
		log.error("Error al guardar el perfil: " + perfil, e);
		FacesUtil.showMessage("Error al guardar el perfil", FacesUtil.ERROR);
	    }
		...
		FacesUtil.showMessage("Perfil actualizado correctamente", FacesUtil.INFO);
		
	    } catch (Exception e) {
		log.error("Error al actualizar el perfil: " + perfil, e);
		FacesUtil.showMessage("Error al actualizar el perfil", FacesUtil.ERROR);
	    }
	}
    }
	...
	FacesUtil.showMessage("No puede eliminar por que este perfil tiene Usuarios", FacesUtil.WARN);
    }
